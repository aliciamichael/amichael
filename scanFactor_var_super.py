...
@cmd.extend
def scanFactor( nucleosome, probe, superLength=2, cutoff=2, clashKeep=1, writeModels=False ):
  """
USAGE

  scanFactor nucleosome object,
             probe object,
             superLength = number of bases on probe to use for superposition (default 2),
             cutoff = distance in Ångstrom below which atoms are considered clashing (default 2),
             keep models with fewer than the specified number of residue clashes (default 1)
  ...
  secondBase = firstBase+int(superLength)
  offset = firstBase-1
  ...
  for chain in chains:
     ...
     for i in range(firstBase, (firstBase + chainLength) ):
        ...
        if not cmd.select("%s and chain %s and resi %d and name P" % (nucleosome, chain, i) ) :
           cmd.select("moving", "%s and chain %s and resi %d and backbone and not (n. P or n. OP1 or n. OP2)" % (probe, int(superLength)) )
        ...
        # Handle overhanging superposition for final base
           ...
           cmd.select("moving", "%s and chain %s and resi 1-%d and backbone" % (probe, probeDNAChain, int(superLength)) )
